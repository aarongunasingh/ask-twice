...
from asktwice.config import BGE_DIMS, HANDLABEL_QUESTIONS, HANDLABEL_ROWS
...
def test_fixture_end_to_end(tmp_path):
    ...
    assert summary["answer_quality"]["judgments"] == min(HANDLABEL_ROWS, 40) * min(HANDLABEL_QUESTIONS, len(load_questions().noul))
